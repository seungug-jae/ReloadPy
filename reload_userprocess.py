...
_numeric_options = ['num_cycles', 'num_past_cycles','num_reload_comp','num_persent_case','peak_power_limit',\
    ...
    'eps_power']
_defined_objectives = ['assembly_power','peak_power','burnup','reactivity','power_shape','power_deep']
...
def reload_output_scan(file):
    ...
    try:
        with open(file,'r') as f:
            ...
            f.seek(0)
            line=skip_to(f,'***** USER EDIT FOR DEVELOPEMENT 1')
            skip_lines(f,1)
            results['tallydat'] = {}
            for i in range(0, 703):
                line = f.readline().split()
                results['tallydat'][int(line[0])] = convert_str2num(line[1:])
        return results
    except Exception:
        return {}
...
if __name__ == "__main__":
    # nout = 101
    # ndetail_srt = 35
    # outdir = './run_Iter6_maxlength_4/'
    # outfile = 'maxlength_4_out.txt'
    #
    nout = 1
    ndetail_srt = 1
    outdir = './run_Iter6_maxlength_persent/'
    outfile = 'maxlength_persent_out.txt'
    #
    output = {}
    line_1 = ' {:8.5f} {:8.5f}'
    line_2 = ' {:12.4e}'*8 + '   ' + ' {:5d}'*2 + ' {:12.4e}'*2
    with open(outfile, 'w') as f:
        f.write('  idx  iass  insertk  addedk    bocpwr       eocpwr       asspwr       pekpwr       pekass       bocden       eocden       pekden       pekasm  node   assbrn       rmsdif       eocpek\n')
        for i in range(ndetail_srt, nout+1):
            output[i] = reload_output_scan(outdir+'reload.out_'+f'{i}')
            for iass in output[i]['worth']:
                f.write(f' {i:4d} {iass:6d}') 
                f.write(line_1.format(*output[i]["worth"][iass]))
                f.write(line_2.format(*output[i]["tallydat"][iass]))
                f.write(f' {output[i]["eoc_pekass"][3]:12.4e}')
                f.write('\n')
    # nargs = len(sys.argv)
    # if nargs < 2:

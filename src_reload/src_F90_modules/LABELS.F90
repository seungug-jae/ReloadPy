...
TYPE LABELS_DATA
   ...
   LABELS_Int :: NTZSZ    ! NUMBER OF ZONES AND SUBZONES
   LABELS_Int :: NREG     ! NUMBER OF REGIONS
   LABELS_Int :: NAREA    ! NUMBER OF AREAS
   LABELS_Int :: LREGA    ! LENGTH OF NRA ARRAY
   LABELS_Int :: NHTS1    ! NUMBER OF HALF-HEIGHT AND EXTRAPOLATION DISTANCE SPECIFICATIONS {0 - NONE,1 - SINGLE VALUE USED EVERYWHERE, .EQ.NREG - REGION DEPENDENT)
   LABELS_Int :: NHTS2    ! NUMBER OF HALF-HEIGHT AND EXTRAPOLATION DISTANCE SPECIFICATIONS FOR THE SECOND DIRECTION. {SAME DEFINITION AS NHTS1}
   LABELS_Int :: NSETS    ! NUMBER OF NUCLIDE SETS
   LABELS_Int :: NALIAS   ! 0 IF RECORD 5D IS NOT PRESENT.  IF GREATER THAN 0, RECORD 5D IS PRESENT AND THE ALIAS ARRAY IS OF LENGTH NALIAS.
   LABELS_Int :: NTRI     ! NO. OF TRIANGLES PER HEX FOR TRIANGULAR GEOMETRIES
   LABELS_Int :: NRING    ! MAX. NO. OF RINGS OF HEXAGONS FOR TRIANGULAR GEOMETRIES
   LABELS_Int :: NCHAN    ! NO. OF CONTROL-ROD CHANNELS IN THE MODEL.  NCHAN IS THE SUM OF NRODS (RECORD 6D) OVER ALL CONTROL ROD BANKS.
   LABELS_Int :: NBANKS   ! NO. OF CONTROL-ROD BANKS
   LABELS_Int :: LINTAX   ! ORIGINAL NO. OF FINE MESH INTERVALS IN AXIAL DIMENSION
   LABELS_Int :: MAXTIM   ! MAXIMUM VALUE OF NTIMES(I) (I=1,NBANKS)
   LABELS_Int :: MAXROD   ! MAXIMUM VALUE OF NRODS(I) (I=1,NBANKS)
   LABELS_Int :: MAXMSH   ! MAXIMUM VALUE OF NMESH(K) (K=1,LRODS;I=1,NBANKS) WHERE LRODS=NRODS(I)
   LABELS_Int :: MAXLRD   ! MAXIMUM VALUE OF LENROD(K) (K=1,LRODS;I=1,NBANKS)
   LABELS_Int :: MAXLCH   ! MAXIMUM VALUE OF LENCHN(K) (K=1,LRODS;I=1,NBANKS)
   LABELS_Int :: NVARY    ! NUMBER OF BURNUP DEPENDENT ISOTOPES
   LABELS_Int :: MAXBRN   ! THE MAXIMUM NUMBER OF GROUPS THAT ARE BURNUP DEPENDENT FOR ANY OF THE BURNUP DEPENDENT ISOTOPES
   LABELS_Int :: MAXORD   ! THE HIGHEST ORDER POLYNOMIAL FIT FOR ANY OF THE GROUPS FOR ANY OF THE BURNABLE ISOTOPES
   LABELS_Int :: ND       ! 1 IN XY AND RZ GEOMETRIES, 2 IN XYZ, R-THETA-Z, THETA-R-Z, HEX-Z, AND TRIANGULAR-Z MODELS
   LABELS_Int :: IDUM(2)  ! RESERVED
   ! CARD TYPE 2
   DIF3D_Char,       DIMENSION(:),POINTER :: CMPNAM  ! NTZSZ    ZONE OR SUBZONE LABEL. SUBZONES FOLLOW ZONES
   DIF3D_Char,       DIMENSION(:),POINTER :: REGNAM  ! NREG     REGION LABEL
   DIF3D_Char,       DIMENSION(:),POINTER :: ARANAM  ! NAREA    AREA LABEL
   LABELS_Int,DIMENSION(:),POINTER :: NRA     ! LREGA    REGION/AREA ASSIGNMENTS.   NRA(N(J)) IS THE NUMBER OF REGIONS IN AREA J.
   ...
   LABELS_Real,DIMENSION(:),POINTER :: HAFHT1  ! NHTS1    ACTUAL TRANSVERSE HALF HEIGHT FOR REGION I
                                                          !          IF HAFHTN(I)+XTRAPN(I)=0.0 (N =1 OR 2) 
                                                          !          THE MODEL IS TREATED AS EXTENDING TO INFINITY IN THE NTH TRANSVERSE DIMENSION.
   LABELS_Real,DIMENSION(:),POINTER :: XTRAP1  ! NHTS1    TRANSVERSE EXTRAPOLATION DISTANCE FOR REGION I
   LABELS_Real,DIMENSION(:),POINTER :: HAFHT2  ! NHTS2    ACTUAL TRANSVERSE HALF HEIGHT FOR REGION I IN THE SECOND DIRECTION
   LABELS_Real,DIMENSION(:),POINTER :: XTRAP2  ! NHTS2    TRANSVERSE EXTRAPOLATION DISTANCE FOR REGION I IN THE SECOND DIRECTION
   ! CARD TYPE 4  NUCLIDE SET LABELS  (PRESENT IF NSETS.GT.1)
   DIF3D_Char,       DIMENSION(:),POINTER :: SETISO  ! NSETS
   ! CARD TYPE 5  ALIAS ZONE LABELS   (PRESENT IF NALIAS .GT. 0)
   DIF3D_Char,       DIMENSION(:),POINTER :: ALIAS   ! NALIAS
   ...
   DIF3D_Char,       DIMENSION(:),POINTER :: BNKLAB  ! NBANKS   CONTROL-ROD BANK LABEL
   LABELS_Real,DIMENSION(:),POINTER :: ZMESHO  ! LINTAX   ORIGINAL LAST-DIMENSION MESH STRUCTURE
   ...
   LABELS_Real,DIMENSION(:),POINTER :: POSBNK  ! NBANKS   CURRENT POSITION OF ROD BANK I
   LABELS_Int,DIMENSION(:),POINTER :: NRODS   ! NBANKS   NO. OF RODS IN BANK I
   LABELS_Int,DIMENSION(:),POINTER :: NTIMES  ! NBANKS   NO. OF TIME NODES IN POSITION VS. TIME TABLE FOR BANK I
   LABELS_Int,DIMENSION(:),POINTER :: KFINTO  ! LINTAX-1    ORIGINAL NUMBER OF FINE MESH BETWEEN ZMESHO(I) AND ZMESHO(I+1)
   ! CARD TYPE 7           CONTROL-ROD BANK DATA (PRESENT IF NBANKS .GT. 0)
   LABELS_Real,DIMENSION(:,:),POINTER :: RBTIME   ! MAXTIM,NBANKS   TIME ENTRIES IN ROD POSITION VS. TIME TABLE
   LABELS_Real,DIMENSION(:,:),POINTER :: RBPOS    ! MAXTIM,NBANKS   ROD-BANK POSITIONS IN TABLE (CM.)
   LABELS_Int,DIMENSION(:,:),POINTER :: NMESH    ! MAXROD,NBANKS   NO. OF PLANAR MESH CELLS IN ROD K OF CURRENT ROD BANK
   LABELS_Int,DIMENSION(:,:),POINTER :: LENCHN   ! MAXROD,NBANKS   NO. OF REGIONS Defined FOR THE IMMOVABLE PORTION OF ROD CHANNEL K
   LABELS_Int,DIMENSION(:,:),POINTER :: LENROD   ! MAXROD,NBANKS   NO. OF REGIONS Defined FOR THE MOVEABLE PORTION OF ROD CHANNEL K
   ! CARD TYPE 8           CONTROL-ROD CHANNEL DATA (PRESENT IF LLCHN + LLROD + MMESH .GT. 0)
   LABELS_Real,DIMENSION(:,:,:),  POINTER :: POSCHN  ! MAXLCH,MAXROD,NBANKS    POSITION (RELATIVE TO THE BOTTOM) OF THE LOWER BOUNDARY OF REGION L 
                                                              !                   IN THE IMMOVABLE PORTION OF THE CURRENT ROD CHANNEL (POSCHN(1)=0.0)
   LABELS_Real,DIMENSION(:,:,:),  POINTER :: POSROD  ! MAXLRD,MAXROD,NBANKS    POSITION (RELATIVE TO ROD TIP) OF THE LOWER BOUNDARY OF REGION L 
                                                              !                   IN THE MOVEABLE PORTION OF THE CURRENT ROD CHANNEL (POSROD(1)=0.0)
   LABELS_Int,DIMENSION(:,:,:),  POINTER :: MRCHN   ! MAXLCH,MAXROD,NBANKS    REGION ASSIGNMENT FOR REGIONS IN THE IMMOVABLE PORTION OF THE CURRENT 
                                                              !                   ROD CHANNEL, STARTING AT THE BOTTOM (Z=0.0) OF THE MODEL.
   LABELS_Int,DIMENSION(:,:,:),  POINTER :: MRROD   ! MAXLRD,MAXROD,NBANKS    REGION ASSIGNMENT FOR REGIONS IN THE MOVEABLE PORTION OF THE ROD, 
                                                              !                   STARTING WITH THE REGION ADJACENT TO THE ROD TIP
   LABELS_Int,DIMENSION(:,:,:,:),POINTER :: MESH    ! 2,MAXMSH,MAXROD,NBANKS  1ST DIMENSION INDEX FOR PLANAR MESH CELL M IN THE CURRENT ROD CHANNEL
   ...
   LABELS_Real,DIMENSION(:),  POINTER :: BRNLAB  ! NVARY     LABELS OF BURNUP DEPENDENT ISOTOPES
   LABELS_Real,DIMENSION(:),  POINTER :: BRNREF  ! NVARY     LABELS OF THE REFERENCE BASE ISOTOPES FOR EACH OF THE BURNUP DEPENDENT ISOTOPES
   LABELS_Real,DIMENSION(:),  POINTER :: BRNMIN  ! NVARY     SMALLEST ATOMIC DENSITY USED FOR THE POLYNOMIAL FIT FOR THE L-TH BURNUP DEPENDENT ISOTOPE
   LABELS_Real,DIMENSION(:),  POINTER :: BRNMAX  ! NVARY     LARGEST ATOMIC DENSITY USED FOR THE POLYNOMIAL FIT FOR THE L-TH BURNUP DEPENDENT ISOTOPE
   LABELS_Int,DIMENSION(:),  POINTER :: NGPVAR  ! NVARY     # OF GROUPS THAT WILL HAVE BURNUP DEPENDENT CROSS SECTIONS FOR EACH OF THE BURNUP DEPENDENT ISOTOPE
   LABELS_Int,DIMENSION(:,:),POINTER :: KINDXS  ! NVARY,2   TYPE OF CROSS SECTIONS WHICH ARE BURNUP DEPENDENT.  
                                                            !           THE BINARY BIT PATTERN OF THIS DIF3D_Int WORD SPECIFIES A LIST OF CROSS SECTION TYPES WHICH ARE BURNUP DEPENDENT.
   ...
   LABELS_Int,DIMENSION(:,:), POINTER :: NUMGRP  ! MAXBRN,NVARY   (I,L) GROUP # FOR THE I-TH GROUP THAT IS BURNUP DEPENDENT FOR THE L-TH BURNUP DEPENDENT ISOTOPE
   LABELS_Int,DIMENSION(:,:), POINTER :: NORCAP  ! MAXBRN,NVARY   (I,L) THE ORDER FOR THE LEAST SQUARES POLYNOMIAL FIT FOR CAPTURE FOR THE 
                                                             !                      I-TH GROUP AND L-TH BURNUP DEPENDENT ISOTOPE
   LABELS_Int,DIMENSION(:,:), POINTER :: NORFIS  ! MAXBRN,NVARY   (I,L) THE ORDER FOR THE LEAST SQUARES POLYNOMIAL FIT FOR FISSION FOR THE
                                                             !                      I-TH GROUP AND L-TH BURNUP DEPENDENT ISOTOPE
   ...
   LABELS_Real,DIMENSION(:,:,:),POINTER :: CFIT  ! MAXORD+1,MAXBRN,NVARY  (I,J,L) COEFFICIENTS IN THE POLYNOMIAL FIT FOR THE CAPTURE CROSS SECTION
   LABELS_Real,DIMENSION(:,:,:),POINTER :: FFIT  ! MAXORD+1,MAXBRN,NVARY  (I,J,L) COEFFICIENTS IN THE POLYNOMIAL FIT FOR THE FISSION CROSS SECTION
END TYPE
...
!---------------------------------------------------------------------------------------------------------------------------------
DIF3D_Int FUNCTION LABELS_IdentifyRegion(User_LABELS,User_RegionName)
IMPLICIT NONE
! Passed in
TYPE (LABELS_DATA) User_LABELS  ! A user data variable to be Defined by WRITEing in a LABELS file
DIF3D_Char         User_RegionName
! Local
DIF3D_Int I,J
...
!---------------------------------------------------------------------------------------------------------------------------------
DIF3D_Int FUNCTION LABELS_IdentifyArea(User_LABELS,User_AreaName)
IMPLICIT NONE
! Passed in
TYPE (LABELS_DATA) User_LABELS  ! A user data variable to be Defined by WRITEing in a LABELS file
DIF3D_Char         User_AreaName
! Local
DIF3D_Int I,J
...
! Card type 2
   ...
   310 FORMAT(('[LABELS]...',8('[',I5,',',I6,']')     ))  ! DIF3D_Int
...
! Local variables
DIF3D_Int :: NTZSZ    ! NUMBER OF ZONES AND SUBZONES
DIF3D_Int :: NREG     ! NUMBER OF REGIONS
DIF3D_Int :: NAREA    ! NUMBER OF AREAS
DIF3D_Int :: LREGA    ! LENGTH OF NRA ARRAY
DIF3D_Int :: NHTS1    ! NUMBER OF HALF-HEIGHT AND EXTRAPOLATION DISTANCE SPECIFICATIONS {0 - NONE,1 - SINGLE VALUE USED EVERYWHERE, .EQ.NREG - REGION DEPENDENT)
DIF3D_Int :: NHTS2    ! NUMBER OF HALF-HEIGHT AND EXTRAPOLATION DISTANCE SPECIFICATIONS FOR THE SECOND DIRECTION. {SAME DEFINITION AS NHTS1}
DIF3D_Int :: NSETS    ! NUMBER OF NUCLIDE SETS
DIF3D_Int :: NALIAS   ! 0 IF RECORD 5D IS NOT PRESENT.  IF GREATER THAN 0, RECORD 5D IS PRESENT AND THE ALIAS ARRAY IS OF LENGTH NALIAS.
DIF3D_Int :: NTRI     ! NO. OF TRIANGLES PER HEX FOR TRIANGULAR GEOMETRIES
DIF3D_Int :: NRING    ! MAX. NO. OF RINGS OF HEXAGONS FOR TRIANGULAR GEOMETRIES
DIF3D_Int :: NCHAN    ! NO. OF CONTROL-ROD CHANNELS IN THE MODEL.  NCHAN IS THE SUM OF NRODS (RECORD 6D) OVER ALL CONTROL ROD BANKS.
DIF3D_Int :: NBANKS   ! NO. OF CONTROL-ROD BANKS
DIF3D_Int :: LINTAX   ! ORIGINAL NO. OF FINE MESH INTERVALS IN AXIAL DIMENSION
DIF3D_Int :: MAXTIM   ! MAXIMUM VALUE OF NTIMES(I) (I=1,NBANKS)
DIF3D_Int :: MAXROD   ! MAXIMUM VALUE OF NRODS(I) (I=1,NBANKS)
DIF3D_Int :: MAXMSH   ! MAXIMUM VALUE OF NMESH(K) (K=1,LRODS;I=1,NBANKS) WHERE LRODS=NRODS(I)
DIF3D_Int :: MAXLRD   ! MAXIMUM VALUE OF LENROD(K) (K=1,LRODS;I=1,NBANKS)
DIF3D_Int :: MAXLCH   ! MAXIMUM VALUE OF LENCHN(K) (K=1,LRODS;I=1,NBANKS)
DIF3D_Int :: NVARY    ! NUMBER OF BURNUP DEPENDENT ISOTOPES
DIF3D_Int :: MAXBRN   ! THE MAXIMUM NUMBER OF GROUPS THAT ARE BURNUP DEPENDENT FOR ANY OF THE BURNUP DEPENDENT ISOTOPES
DIF3D_Int :: MAXORD   ! THE HIGHEST ORDER POLYNOMIAL FIT FOR ANY OF THE GROUPS FOR ANY OF THE BURNABLE ISOTOPES
DIF3D_Int :: ND       ! 1 IN XY AND RZ GEOMETRIES, 2 IN XYZ, R-THETA-Z, THETA-R-Z, HEX-Z, AND TRIANGULAR-Z MODELS
...
#ifdef DIF3D_Debug
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF ZONES AND SUBZONES...........................",33("."),I16)') NTZSZ
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF REGIONS......................................",33("."),I16)') NREG 
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF AREAS........................................",33("."),I16)') NAREA
   WRITE(MODULE_OUT,'("[LABELS]...LENGTH OF NRA ARRAY....................................",33("."),I16)') LREGA
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF HALF-HEIGHT AND EXTPOL. DIST. SPECS D=1......",33("."),I16)') NHTS1
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF HALF-HEIGHT AND EXTPOL. DIST. SPECS D=2......",33("."),I16)') NHTS2
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF NUCLIDE SETS.................................",33("."),I16)') NSETS
   WRITE(MODULE_OUT,'("[LABELS]...LENGTH OF ALIAS ARRAY..................................",33("."),I16)') NALIAS
   WRITE(MODULE_OUT,'("[LABELS]...NO. OF TRIANGLES PER HEX FOR TRIANGULAR GEOMETRIES.....",33("."),I16)') NTRI  
   WRITE(MODULE_OUT,'("[LABELS]...MAX. NO. OF RINGS OF HEXAGONS FOR TRIANGULAR GEOMETRIES",33("."),I16)') NRING 
   WRITE(MODULE_OUT,'("[LABELS]...NO. OF CONTROL-ROD CHANNELS IN THE MODEL...............",33("."),I16)') NCHAN 
   WRITE(MODULE_OUT,'("[LABELS]...NO. OF CONTROL-ROD BANKS...............................",33("."),I16)') NBANKS
   WRITE(MODULE_OUT,'("[LABELS]...ORIGINAL NO. OF FINE MESH INTERVALS IN AXIAL DIMENSION.",33("."),I16)') LINTAX
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM TIME...........................................",33("."),I16)') MAXTIM
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM NUMBER OF CONTROL RODS.........................",33("."),I16)') MAXROD
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM NUMBER OF MESHES...............................",33("."),I16)') MAXMSH
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM NUMBER OF CONTROL ROD PIECES...................",33("."),I16)') MAXLRD
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM NUMBER OF CONTROL ROD CHANNELS?................",33("."),I16)') MAXLCH
   WRITE(MODULE_OUT,'("[LABELS]...NUMBER OF BURNUP DEPENDENT ISOTOPES....................",33("."),I16)') NVARY
   WRITE(MODULE_OUT,'("[LABELS]...MAXIMUM NUMBER OF BURNUP DEPENDENT GROUPS..............",33("."),I16)') MAXBRN
   WRITE(MODULE_OUT,'("[LABELS]...HIGHEST ORDER POLYNOMIAL FIT...........................",33("."),I16)') MAXORD
   WRITE(MODULE_OUT,'("[LABELS]...DIMENSIONS? 1 OR 2.....................................",33("."),I16)') ND
#endif

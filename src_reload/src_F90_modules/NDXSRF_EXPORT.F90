!---------------------------------------------------------------------------------------------------------------------------------
!  NDXSRF_EXPORT(USER_NDXSRF,USER_FILENAME)
!  EXPORTs the NDXSRF data file
!---------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE NDXSRF_EXPORT(Output_Unit,USER_NDXSRF,USER_FILENAME)
USE NDXSRF_io
USE NE_Kind
...
102 FORMAT('')
105 FORMAT('[NDXSRF]...THERE WAS A FATAL ERROR THAT OCCURED IN (EXPORT)',56('.'))

IF (.NOT. USER_NDXSRF%DEFINED) THEN  ! Data to write doesn't exist
   WRITE(Output_Unit,105)
   WRITE(Output_Unit,'("[NDXSRF]...USER DATA WAS UNDEFINED FOR EXPORT <",A60,">",7("."))') USER_FILENAME
...
END IF
   ...
   WRITE(Output_Unit,'("[DLAYXS]...DEBUG PRINT OF NDXSRF EXPORT ROUTINE",52("."),A16)') USER_FILENAME
   WRITE(Output_Unit,101)
#endif
! WRITE Card Type 0
WRITE(OUTPUTUNIT) USER_NDXSRF%HNAME,(USER_NDXSRF%HUSE(I),I=1,2),USER_NDXSRF%IVERS
! WRITE Card Type 1
WRITE(OUTPUTUNIT) USER_NDXSRF%NON,USER_NDXSRF%NSN,USER_NDXSRF%NNS,USER_NDXSRF%NAN,USER_NDXSRF%NZONE,USER_NDXSRF%NSZ
...
! WRITE Card Type 2
WRITE(OUTPUTUNIT) (USER_NDXSRF%HNNAME(N),N=1,USER_NDXSRF%NON),(USER_NDXSRF%HANAME(N),N=1,USER_NDXSRF%NON),       &
   ...
   DO J = 1,USER_NDXSRF%NSN
      ...
      WRITE(Output_Unit,'(("[NDXSRF]...",8("[",I4,",",I4,"]",1X),8(".") ))') ((I,USER_NDXSRF%NOS(I,J)),I=1,USER_NDXSRF%NDXS(1,J))
      WRITE(Output_Unit,'("[NDXSRF]...NOR[I,#] ORDER # IN THIS SET OF NUCLIDE I FROM CROSS SECTION DATA (HNNAME)",33("."))')
      WRITE(Output_Unit,'(("[NDXSRF]...",8("[",I4,",",I4,"]",1X),8(".") ))') ((I,USER_NDXSRF%NOR(I,J)),I=1,USER_NDXSRF%NNS)
   END DO
#endif

! WRITE Card Type 3
WRITE(OUTPUTUNIT) (USER_NDXSRF%VOLZ(N),N=1,USER_NDXSRF%NZONE),(USER_NDXSRF%VFPA(N),N=1,USER_NDXSRF%NZONE), &
